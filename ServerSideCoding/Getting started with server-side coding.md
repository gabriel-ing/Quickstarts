...
The code used in IRIS is organised in classes (file extension `.cls`). 
...
Globals are the storage structure at the heart of IRIS. These are hierarchical multi-dimensional arrays where each node of the hierarchical tree structure has a key, or subscript and a value. Globals are denoted by a caret `^`, e.g. `^GlobalName`.
...
As of 2022, Python can be used in IRIS classes with no significant performance cost. To use Embedded Python, simply add the keyword tag `[language=python]` after an iris method or class method. For example: 

 While this is generally the most effective method to use Python with IRIS, you can also connect external applications to iris, see the external language page for more detail (**Add link to external language quickstart**)
...
You can install and use Python packages in IRIS using the following in the bash terminal:
