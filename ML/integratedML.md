...
The install `--target` will vary by install method. The target given above is true for an InterSystems IRIS Community image in a docker container, but may need to be altered for other install methods. The general target is `/<IRIS-INSTALL-LOCATION>/mgr/python`. 
...
If you are already in the container from the `pip` install, just run `iris session iris` 
...
This will install the InterSystems Package Manager (IPM), this is used through the command `zpm`. To install the dataset, run:
...
In our titanic dataset, there are 891 passenger records, so we will take the records with PassengerId as a multiple of 5 as the trainng dataset (~80%) and rest as the testing set (~20%). Here we create `VIEWS`, which are virtual tables that can be treated like a standard table, but do not duplicate the data.

```sql
CREATE VIEW dc_data.TitanicTrain AS
SELECT * 
FROM dc_data.Titanic
WHERE MOD(PassengerId, 5) IN (1,2,3,4)
```
This selects passengers where passenger ID is not a multiple of 5
```sql
...
It is possible to define which columns to use for the prediction using the WITH keyword, this is not required as the AutoML model will automatically find and use the relevant columns. If you do define the columns used, you must specify data types and ensure those columns exist with compatible types at predict time.
...
WHERE MODEL_NAME = 'TitanicSurvival'
...
Model validation should still be run in the same way as for classification. Here, the validation gave a Root Mean Squared Error of 43.90, and a R^2 value of 0.70. This that 70% of the variance in the data is accounted for by the model, which is pretty good.
